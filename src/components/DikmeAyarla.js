...
const DikmeAyarla = (mobilya, bolge_x, bolge_y, secilenBolge, islem) => {
  ...
  for (let index = 0; index < mobilya.dikme.length; index++) {
    if (dikme[index].y1 >= bolge[secilenBolge].y2 && dikme[index].y3 <= bolge[secilenBolge].y4 && bolge[secilenBolge].x2 === dikme[index].x1) {
      secilenSagDikme_index = index
      secilenSagDikme = dikme[index]
    }

    if (dikme[index].y2 >= bolge[secilenBolge].y1 && dikme[index].y4 <= bolge[secilenBolge].y3 && bolge[secilenBolge].x1 === dikme[index].x2) {
  ...
  //@sag_dikme işlemi--------------------------------------------------------------
  console.log('mobilya.bolge[secilenBolge].x=', mobilya.bolge[secilenBolge].x, 'bolge_x=', bolge_x)
  if (islem === 'sag_dikme' && bolge_x > 0) {
    ...
    for (let index = 0; index < bolge.length; index++) {
      if (secilenSagDikme.y1 >= bolge[index].y2 && secilenSagDikme.y3 <= bolge[index].y4 && secilenSagDikme.x1 === bolge[index].x2) {
        komsu_sol_bolge.push(index)
        say = say + 1
      }
      if (secilenSagDikme.y2 >= bolge[index].y1 && secilenSagDikme.y4 <= bolge[index].y3 && secilenSagDikme.x2 === bolge[index].x1) {
    ...
    let artir1 = 0
    for (let index = 0; index < mobilya.raf.length; index++) {
      if (raf[index].y2 <= dikme[secilenSagDikme_index].y1 && raf[index].y4 >= dikme[secilenSagDikme_index].y3 && raf[index].x2 === dikme[secilenSagDikme_index].x1) {
        komsuSolRaf.push(index)
        artir = artir + 1
      }
      if (raf[index].y1 <= dikme[secilenSagDikme_index].y2 && raf[index].y3 >= dikme[secilenSagDikme_index].y4 && raf[index].x1 === dikme[secilenSagDikme_index].x2) {
    ...
    for (let index = 0; index < komsu_sol_bolge.length; index++) {
      if (bolge_x > 0) {
        ...
        mobilya.bolge[komsu_sol_bolge[index]].cx = mobilya.bolge[komsu_sol_bolge[index]].cx - sonuc / 2
        mobilya.bolge[komsu_sol_bolge[index]].x = mobilya.bolge[komsu_sol_bolge[index]].x - sonuc
      }
    }
    for (let index = 0; index < komsu_sag_bolge.length; index++) {
      if (bolge_x > 0) {
        mobilya.bolge[komsu_sag_bolge[index]].cx = mobilya.bolge[komsu_sag_bolge[index]].cx - sonuc / 2
        mobilya.bolge[komsu_sag_bolge[index]].x = mobilya.bolge[komsu_sag_bolge[index]].x + sonuc
      }
    }
    ...
    if (secilenSagDikme === 'yok') {
      ...
      for (let index = 0; index < komsuSagRaf.length; index++) {
        mobilya.raf[komsuSagRaf[index]].x0 = mobilya.raf[komsuSagRaf[index]].x0 - sonuc / 2
        mobilya.raf[komsuSagRaf[index]].x = mobilya.raf[komsuSagRaf[index]].x + sonuc
      }
      for (let index = 0; index < komsuSolRaf.length; index++) {
        mobilya.raf[komsuSolRaf[index]].x0 = mobilya.raf[komsuSolRaf[index]].x0 - sonuc / 2
        mobilya.raf[komsuSolRaf[index]].x = mobilya.raf[komsuSolRaf[index]].x - sonuc
      }
      //rafın ayarlanması
      mobilya.dikme[secilenSagDikme_index].x0 = mobilya.dikme[secilenSagDikme_index].x0 - sonuc
    }
  }
  //----------------------------------------------------------------------------
  //@sol_dikme--------------------------------------------------------------------
  if (islem === 'sol_dikme' && bolge_x > 0) {
    ...
    let say1 = 0
    for (let index = 0; index < bolge.length; index++) {
      if (secilenSolDikme.y2 >= bolge[index].y1 && secilenSolDikme.y4 <= bolge[index].y3 && secilenSolDikme.x2 === bolge[index].x3) {
        komsu_sag_bolge.push(index)
        say = say + 1
      } else {
      }
      if (secilenSolDikme.y1 >= bolge[index].y2 && secilenSolDikme.y3 <= bolge[index].y4 && secilenSolDikme.x1 === bolge[index].x2) {
        komsu_sol_bolge.push(index)
        say1 = say1 + 1
      }
    }
    ...
    let artir1 = 0
    for (let index = 0; index < mobilya.raf.length; index++) {
      if (raf[index].y2 <= dikme[secilenSolDikme_index].y1 && raf[index].y4 >= dikme[secilenSolDikme_index].y3 && raf[index].x2 === dikme[secilenSolDikme_index].x1) {
        komsuSolRaf.push(index)
        artir = artir + 1
      }
      if (raf[index].y1 <= dikme[secilenSolDikme_index].y2 && raf[index].y3 >= dikme[secilenSolDikme_index].y4 && raf[index].x1 === dikme[secilenSolDikme_index].x2) {
        komsuSagRaf.push(index)
        artir1 = artir1 + 1
      }
    }
    ...
    if (secilenSolDikme === 'yok') {
      ...
      mobilya.dikme[secilenSolDikme_index].x0 = mobilya.dikme[secilenSolDikme_index].x0 + sonuc
    }
  }
